...
def gamestate_vis_string(state):
        ...
        seat = [VIS_STR_SEAT[player.seat] for player in state.players]
        hands = [pad(tiles_to_strs(player.hand), 14) for player in state.players]
        discs = [pad(tiles_to_strs(player.discards), 24) for player in state.players]
        dora = pad([VIS_STR_FACES[face] for face in state.wall.dora()], 5)
        uradora = pad([VIS_STR_FACES[face] for face in state.wall.uradora()], 5)
        return (
