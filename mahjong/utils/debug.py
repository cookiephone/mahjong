...
def gamestate_vis_string(state):
        ...
        pad = lambda lst, sz, val: lst + [val] * (sz - len(lst))
        seat = [VIS_STR_SEAT[player.seat] for player in state.hands[-1].players]
        hands = [pad(tiles_to_strs(player.hand), 14, "  ") for player in state.hands[-1].players]
        discs = [pad(tiles_to_strs(player.discards), 24, "  ") for player in state.hands[-1].players]
        dora = pad([VIS_STR_FACES[face] for face in state.hands[-1].wall.dora()], 5, "  ")
        uradora = pad([VIS_STR_FACES[face] for face in state.hands[-1].wall.uradora()], 5, "  ")
        melds = [[], [], [], []]
