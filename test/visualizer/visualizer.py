...
class Visualizer(arcade.Window):
    ...
    def __init__(self):
        super().__init__(width=config.SCREEN_WIDTH, height=config.SCREEN_HEIGHT, title=config.SCREEN_TITLE, update_rate=config.UPDATE_RATE)
        self.center_x = self.width // 2
        self.center_y = self.height // 2
        mock = arcade.Sprite(filename=config.TILE_IMAGES["front"], scale=config.TILE_SCALING)
    ...
    def add_tile_row(self, tiles=[], anchor_x=0, anchor_y=0, rotation=0, spacing=2, **kwargs):
        ...
        stride = self.tile_width + spacing
        stride_x, stride_y = {0: (stride, 0), 90: (0, stride), 180: (-stride, 0), 270: (0, -stride)}[rotation]
    ...
    def add_discard_pile(self, player, tiles=[]):
        ...
        rx, ry = {"bottom": (0, row_offset), "right": (-row_offset, 0), "top": (0, -row_offset), "left": (row_offset, 0)}[player]
        self.add_tile_row(tiles[:6], anchor_x=self.center_x - dx, anchor_y=self.center_y - dy, rotation=rotation)
    ...
    def add_hand(self, player, tiles=[], draw=None):
        ...
        anchor_x, anchor_y = self.center_x - dx, self.center_y - dy
        self.add_tile_row(tiles[:13], anchor_x=anchor_x, anchor_y=anchor_y, rotation=rotation)
        if draw:
    ...
    def update_gui(self):
        self.texts = []
        self.texts.append(arcade.Text("East", self.center_x - 90, self.center_y - 90, color=arcade.csscolor.WHITE, rotation=0))
        self.texts.append(arcade.Text("South", self.center_x + 90, self.center_y - 90, color=arcade.csscolor.WHITE, rotation=90))
        self.texts.append(arcade.Text("West", self.center_x + 90, self.center_y + 90, color=arcade.csscolor.WHITE, rotation=180))
        self.texts.append(arcade.Text("North", self.center_x - 90, self.center_y + 90, color=arcade.csscolor.WHITE, rotation=-90))
        self.texts.append(arcade.Text("30000", self.center_x, self.center_y - 60, color=arcade.csscolor.WHITE, rotation=0, font_size=18, anchor_x="center", anchor_y="center"))
        self.texts.append(arcade.Text("30000", self.center_x + 60, self.center_y, color=arcade.csscolor.WHITE, rotation=90, font_size=18, anchor_x="center", anchor_y="center"))
        self.texts.append(arcade.Text("30000", self.center_x, self.center_y + 60, color=arcade.csscolor.WHITE, rotation=180, font_size=18, anchor_x="center", anchor_y="center"))
        self.texts.append(arcade.Text("30000", self.center_x - 60, self.center_y, color=arcade.csscolor.WHITE, rotation=-90, font_size=18, anchor_x="center", anchor_y="center"))
        self.texts.append(arcade.Text("East 1", self.center_x, self.center_y + 15, color=arcade.csscolor.WHITE, font_size=20, anchor_x="center", anchor_y="center"))
        self.texts.append(arcade.Text("x35", self.center_x, self.center_y - 15, color=arcade.csscolor.WHITE, font_size=15, anchor_x="center", anchor_y="center"))

    def draw_gui(self):
        self.gui_camera.use()
        arcade.draw_lrtb_rectangle_outline(self.center_x - 100, self.center_x + 100, self.center_y + 100, self.center_y - 100, arcade.csscolor.WHITE, border_width=3)
